...
def main() -> None:
    ...
    while True:
        try:
            size = int(input("Введите размер поля (размер должен быть >= 3 и <= 10): "))
            while not (3 <= size <= 10):
                size = int(input("Размер должен быть больше или равен 3" \
                                 " и меньше или равен 10. Повторите попытку "))
            break
        except ValueError:
