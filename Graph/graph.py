...
from Graph.Iterators.edgeiterators import (
    BiDirectionalEdgeIterator,
    ConstBiDirectionalEdgeIterator,
)
from Graph.Iterators.vertexiterators import (
    BiDirectionalVertexIterator,
    ConstBiDirectionalVertexIterator,
)
import logging
import copy
...
class Graph(Generic[T]):
    ...
    def __deepcopy__(self, memo: dict) -> "Graph[T]":
        ...
        for v1, v2 in self._edges:
            new_graph.add_edge(
                copy.deepcopy(v1.value, memo), copy.deepcopy(v2.value, memo)
            )
    ...
    def adjacency_vertex_iterator(
        self, vertex_value: T
    ) -> BiDirectionalVertexIterator[T]:
        """
    ...
    def reverse_adjacency_vertex_iterator(
        self, vertex_value: T
    ) -> BiDirectionalVertexIterator[T]:
        """
    ...
    def const_adjacency_vertex_iterator(
        self, vertex_value: T
    ) -> ConstBiDirectionalVertexIterator[T]:
        """
    ...
    def const_reverse_adjacency_vertex_iterator(
        self, vertex_value: T
    ) -> ConstBiDirectionalVertexIterator[T]:
        """
    ...
    def reverse_incident_edge_iterator(
        self, vertex_value: T
    ) -> BiDirectionalEdgeIterator[T]:
        """
    ...
    def const_incident_edge_iterator(
        self, vertex_value: T
    ) -> ConstBiDirectionalEdgeIterator[T]:
        """
    ...
    def const_reverse_incident_edge_iterator(
        self, vertex_value: T
    ) -> ConstBiDirectionalEdgeIterator[T]:
        """
    ...
    def remove_vertex_by_iterator(
        self, iterator: BiDirectionalVertexIterator[T]
    ) -> None:
        """
    ...
    def __eq__(self, other) -> bool:
        ...
        self_edges = {
            tuple(sorted([edge[0].value, edge[1].value])) for edge in self._edges
        }
        other_edges = {
            tuple(sorted([edge[0].value, edge[1].value])) for edge in other._edges
        }
    ...
    def __str__(self) -> str:
        ...
        graph_edges = ", ".join(
            [f"{edge[0].value}->{edge[1].value}" for edge in self._edges]
        )
